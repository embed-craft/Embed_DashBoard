...
import { Layer, FullScreenConfig, LayerStyle } from '@/store/useEditorStore';
...
interface FullScreenRendererProps {
    ...
    onConfigChange?: (config: FullScreenConfig) => void;
...
export const FullScreenRenderer: React.FC<FullScreenRendererProps> = ({
    ...
    // FIX: select root layer (no parent) instead of looking for 'modal' type
    const modalLayer = layers.find(l => !l.parent) || layers[0] || { id: 'fallback', type: 'container', content: {}, style: {} } as any;
    // --- TIMING LOGIC ---
    const [isVisible, setIsVisible] = useState(config?.timing?.delay === 0);

    // Handle Delay
    useEffect(() => {
        if (!config?.timing?.delay) {
            setIsVisible(true);
            return;
        }
        const timer = setTimeout(() => {
            setIsVisible(true);
        }, config.timing.delay * 1000); // Convert seconds to ms
        return () => clearTimeout(timer);
    }, [config?.timing?.delay]);

    // Handle Duration (Auto Dismiss)
    useEffect(() => {
        if (!config?.timing?.duration || !isVisible || !isInteractive) return;

        // Don't auto-dismiss if duration is 0 (infinite)
        if (config.timing.duration <= 0) return;

        const timer = setTimeout(() => {
            if (onDismiss) onDismiss();
        }, config.timing.duration * 1000);

        return () => clearTimeout(timer);
    }, [config?.timing?.duration, isVisible, isInteractive, onDismiss]);
        ...
        <>
            ...
            <div
                ...
                style={{
                    ...
                    backgroundColor: config?.backgroundColor || modalLayer.style?.backgroundColor || '#FFFFFF',
                    ...
                    // Force Cover
                    backgroundSize: 'cover', // Full screen usually wants cover
                    ...
                    display: 'block',
                    padding: 0,
                    animation: 'fullscreen-fade 0.3s ease-out'
                }}
            >
                {/* --- MEDIA LAYER (Background) --- */}
                {config?.media?.url && config.media.type !== 'none' && (
                    <div style={{
                        position: 'absolute',
                        top: 0,
                        left: 0,
                        ...
                        zIndex: 0,
                        overflow: 'hidden',
                        pointerEvents: 'none' // Allow clicks to pass through to overlay/content unless controls added
                    }}>
                        {/* YOUTUBE */}
                        {(config.media.type === 'youtube' || config.media.url.includes('youtu')) ? (
                            <iframe
                                src={`https://www.youtube.com/embed/${config.media.url.match(/(?:youtu\.be\/|youtube\.com\/.*v=|embed\/|v\/)([^&?]*)/)?.[1] || ''}?autoplay=${config.media.autoPlay ? 1 : 0}&mute=${config.media.muted ? 1 : 0}&controls=0&loop=${config.media.loop ? 1 : 0}&playlist=${config.media.url.match(/(?:youtu\.be\/|youtube\.com\/.*v=|embed\/|v\/)([^&?]*)/)?.[1] || ''}&playsinline=1&modestbranding=1&rel=0`}
                                style={{
                                    width: '100%',
                                    height: '100%',
                                    border: 'none',
                            ...
                            />
                        ) : (config.media.type === 'video' || config.media.url.endsWith('.mp4')) ? (
                            /* OPTIMIZED VIDEO */
                            <video
