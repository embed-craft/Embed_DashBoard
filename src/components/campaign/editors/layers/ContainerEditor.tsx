...
export const ContainerEditor: React.FC<ContainerEditorProps> = ({
    ...
    const handleLayoutModeChange = (mode: 'free' | 'auto') => {
        onStyleUpdate('layoutMode', mode);
        
        // Sync children positions
        if (layer.children && layer.children.length > 0) {
            const updateLayerStyle = useEditorStore.getState().updateLayerStyle;
            layer.children.forEach(childId => {
                if (mode === 'free') {
                    updateLayerStyle(childId, { position: 'absolute' });
                } else {
                    updateLayerStyle(childId, { position: 'relative', top: undefined, left: undefined });
                }
            });
        }
    };
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-6">
                        ...
                        <div className="space-y-4 border rounded-lg p-3 bg-gray-50/50">
                            ...
                            <div>
                                ...
                                <div className="flex bg-gray-100 p-1 rounded-md border border-gray-200">
                                    {['visible', 'hidden', 'scroll'].map((opt) => (
                                        <button
                                            key={opt}
                                            onClick={() => {
                                                onStyleUpdate('overflow', opt);
                                                if (opt !== 'scroll') {
                                                    onStyleUpdate('overflowX', undefined);
                                                    onStyleUpdate('overflowY', undefined);
                                                    onStyleUpdate('hideScrollbar', undefined);
                                                } else {
                                                    // Default to Vertical when scroll is first clicked
                                                    onStyleUpdate('overflowX', 'hidden');
                                                    onStyleUpdate('overflowY', 'scroll');
                                                    // Default hide scrollbar
                                                    onStyleUpdate('hideScrollbar', true);
                                                }
                                            }}
                                ...
                                {style.overflow === 'scroll' && (
                                    <div className="flex flex-col gap-2 bg-indigo-50 p-2 rounded-md border border-indigo-100 mt-2 animate-in fade-in slide-in-from-top-1">
                                        <div className="flex">
                                            {[
                                                { label: 'Vertical', x: 'hidden', y: 'scroll' },
                                                { label: 'Horizontal', x: 'scroll', y: 'hidden' },
                                                { label: 'Both', x: 'scroll', y: 'scroll' }
                                            ].map((opt) => {
                                                const isActive = (style.overflowX || 'hidden') === opt.x && (style.overflowY || 'scroll') === opt.y;
                                                return (
                                                    <button
                                                        key={opt.label}
                                                        onClick={() => {
                                                            onStyleUpdate('overflowX', opt.x);
                                                            onStyleUpdate('overflowY', opt.y);
                                                        }}
                                                        className={`flex-1 py-1 text-[10px] rounded-sm transition-all ${isActive
                                        ...
                                        <div className="flex items-center justify-between pt-1 border-t border-indigo-100/50">
                                            <span className="text-[10px] text-indigo-700 font-medium tracking-wide uppercase">Hide Scrollbar Theme</span>
                                            <Switch
                                                checked={style.hideScrollbar ?? true}
                                                onCheckedChange={(c) => onStyleUpdate('hideScrollbar', c)}
                                                className="scale-75 origin-right"
                                            />
                                        </div>
                                        {!(style.hideScrollbar ?? true) && (
                                            <div className="space-y-2 pt-2 border-t border-indigo-100/50">
                                                <div className="flex items-center justify-between">
                                                    <span className="text-[10px] text-indigo-700 font-medium tracking-wide">Thumb Color</span>
                                                    <div className="flex gap-2 items-center">
                                                        <button 
                                                            onClick={() => onStyleUpdate('scrollbarThumbColor', 'transparent')}
                                                            title="Transparent / Clear"
                                                            className="text-gray-400 hover:text-red-500 transition-colors"
                                                        >
                                                            <Ban size={14} />
                                                        </button>
                                                        <div className="relative w-5 h-5 rounded overflow-hidden border border-indigo-200">
                                                            <input
                                                                type="color"
                                                                className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
                                                                value={style.scrollbarThumbColor || '#cbd5e1'}
                                                                onChange={e => onStyleUpdate('scrollbarThumbColor', e.target.value)}
                                                            />
                                                            <div className="w-full h-full" style={{ backgroundColor: style.scrollbarThumbColor || '#cbd5e1' }} />
                                                        </div>
                                                    </div>
                                                </div>
                                                <div className="flex items-center justify-between">
                                                    <span className="text-[10px] text-indigo-700 font-medium tracking-wide">Track Color</span>
                                                    <div className="flex gap-2 items-center">
                                                        <button 
                                                            onClick={() => onStyleUpdate('scrollbarTrackColor', 'transparent')}
                                                            title="Transparent / Clear"
                                                            className="text-gray-400 hover:text-red-500 transition-colors"
                                                        >
                                                            <Ban size={14} />
                                                        ...
                                                        <div className="relative w-5 h-5 rounded overflow-hidden border border-indigo-200">
                                                            <input
                                                                type="color"
                                                                className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
                                                                value={style.scrollbarTrackColor || '#f1f5f9'}
                                                                onChange={e => onStyleUpdate('scrollbarTrackColor', e.target.value)}
                                                            />
                                                            <div className="w-full h-full" style={{ backgroundColor: style.scrollbarTrackColor || '#f1f5f9' }} />
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                    ...
                    <TabsContent value="layout" className="space-y-6 mt-0 animate-in fade-in-50">
                        ...
                        <div className="space-y-4 border rounded-lg p-3 bg-gray-50/50">
                            ...
                            {/* Layout Mode Toggle */}
                            <div className="mb-4">
                                <Label className="text-[10px] text-gray-500 mb-2 block font-semibold uppercase tracking-wider">Layout Mode</Label>
