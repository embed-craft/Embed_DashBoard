...
export const MediaRenderer: React.FC<MediaRendererProps> = ({ layer, scale = 1, scaleY = 1, muted = true, isActive = false }) => {
    ...
    const resolvedImageUrl = resolveText(imageUrl);
    const youtubeId = getYouTubeId(resolvedImageUrl);
    const isVideo = resolvedImageUrl.toLowerCase().match(/\.(mp4|webm|ogg)$/);
    ...
    const cleanUrl = resolvedImageUrl.split('?')[0].toLowerCase();
