  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50 overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* ... (Rest of the editor layout - Sidebar, Canvas, Properties) ... */}
        {/* I need to make sure I don't overwrite the complex layout logic. 
            I will use a more targeted replacement in the next step since this file is huge.
        */}
      </div>
    </div>
  );
...
useEffect(() => {
  console.log('DesignStep mounted - Initializing AutoSave...');
  initializeAutoSave();
  return () => {
    ...
    case 'bottomsheet':
      ...
      return (
        <BottomSheetRenderer
          ...
          onLayerUpdate={updateLayerStyle}
        />
      );
    ...
    <div style={{ marginBottom: '20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '12px' }}>
        {[
          ...
          { value: 'dismiss', label: 'Dismiss', icon: <X size={16} /> },
          { value: 'link', label: 'Open Link', icon: <Link size={16} /> },
          { value: 'navigate', label: 'Navigate', icon: <Compass size={16} /> },
          { value: 'submit', label: 'Submit', icon: <Send size={16} /> },
          { value: 'custom', label: 'Custom', icon: <Code size={16} /> }
        ].map((option) => {
          const isSelected = (selectedLayerObj.content?.action || 'none') === option.value;
          return (
            <button
              key={option.value}
              onClick={() => handleContentUpdate('action', option.value)}
              style={{
                display: 'flex',
                flexDirection: 'column',
                alignItems: 'center',
                ...
                gap: '6px',
                padding: '10px',
                border: `1px solid ${isSelected ? colors.primary[500] : colors.border.default}`,
                borderRadius: '8px',
      ...
      {selectedLayerObj.content?.action === 'link' && (
        <div style={{ marginBottom: '12px', animation: 'fadeIn 0.2s ease-in-out' }}>
          ...
          <div style={{ position: 'relative' }}>
            <Link size={14} color={colors.text.tertiary} style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)' }} />
            <input
              type="text"
              value={selectedLayerObj.content?.url || ''}
      ...
      {selectedLayerObj.content?.action === 'navigate' && (
        <div style={{ marginBottom: '12px', animation: 'fadeIn 0.2s ease-in-out' }}>
          <label style={{ display: 'block', fontSize: '12px', color: colors.text.secondary, marginBottom: '4px' }}>Screen Name</label>
          <div style={{ position: 'relative' }}>
            <Compass size={14} color={colors.text.tertiary} style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)' }} />
            <input
              type="text"
              value={selectedLayerObj.content?.screenName || ''}
              onChange={(e) => handleContentUpdate('screenName', e.target.value)}
      ...
      {selectedLayerObj.content?.action === 'custom' && (
        <div style={{ marginBottom: '12px', animation: 'fadeIn 0.2s ease-in-out' }}>
          <label style={{ display: 'block', fontSize: '12px', color: colors.text.secondary, marginBottom: '4px' }}>Event Name</label>
          <div style={{ position: 'relative' }}>
            <Code size={14} color={colors.text.tertiary} style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)' }} />
            <input
              type="text"
              value={selectedLayerObj.content?.actionName || ''}
              onChange={(e) => handleContentUpdate('actionName', e.target.value)}
...
const renderLayerProperties = () => {
  ...
  const renderBottomSheetConfig = () => {
    ...
    return (
      <>
        ...
        {/* Layout Mode (Fix: Stack vs Freeform) */}
        {config.mode !== 'image-only' && (
          <div style={{ marginBottom: '20px', paddingBottom: '16px', borderBottom: `1px solid ${colors.border.default}` }}>
            <h5 style={{ margin: '0 0 12px 0', fontSize: '13px', fontWeight: 600, color: colors.text.primary }}>Layout Mode</h5>
            <div style={{ display: 'flex', gap: '8px', backgroundColor: colors.gray[50], padding: '4px', borderRadius: '8px' }}>
              <button
                onClick={() => handleConfigUpdate('layoutMode', 'stack')}
                style={{
                  flex: 1,
                  padding: '8px',
                  border: 'none',
                  borderRadius: '6px',
                  background: (config.layoutMode || 'stack') === 'stack' ? 'white' : 'transparent',
                  color: (config.layoutMode || 'stack') === 'stack' ? colors.primary[600] : colors.text.secondary,
                  fontSize: '12px',
                  fontWeight: 500,
                  cursor: 'pointer',
                  boxShadow: (config.layoutMode || 'stack') === 'stack' ? '0 1px 2px rgba(0,0,0,0.05)' : 'none',
                  transition: 'all 0.2s'
                }}
              >
                Stack (Flex)
              ...
              <button
                onClick={() => handleConfigUpdate('layoutMode', 'freeform')}
                style={{
                  flex: 1,
                  padding: '8px',
                  border: 'none',
                  borderRadius: '6px',
                  background: config.layoutMode === 'freeform' ? 'white' : 'transparent',
                  color: config.layoutMode === 'freeform' ? colors.primary[600] : colors.text.secondary,
                  fontSize: '12px',
                  fontWeight: 500,
                  cursor: 'pointer',
                  boxShadow: config.layoutMode === 'freeform' ? '0 1px 2px rgba(0,0,0,0.05)' : 'none',
                  transition: 'all 0.2s'
                }}
              >
                Freeform
            ...
            <div style={{ marginTop: '8px', fontSize: '11px', color: colors.text.secondary, lineHeight: '1.4' }}>
              {(config.layoutMode || 'stack') === 'stack'
                ? 'Items are stacked vertically. Good for lists.'
                : 'Items can be placed anywhere. Good for custom designs.'}
            </div>
          </div>
        )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: '4px' }}>
            {[0, 1, 2, 3, 4, 5].map((level) => (
              <button
                ...
                style={{
                  padding: '8px 4px',
                  border: `1px solid ${(config.elevation || 0) === level ? colors.primary[500] : colors.border.default}`,
                  borderRadius: '4px',
                  fontSize: '11px',
                  cursor: 'pointer',
        ...
        <div style={{ marginBottom: '20px', paddingBottom: '16px', borderBottom: `1px solid ${colors.border.default}` }}>
          ...
          <div style={{ marginBottom: '12px' }}>
            ...
            <select
              ...
              onChange={(e) => handleNestedConfigUpdate('animation', 'type', e.target.value)}
              style={{ width: '100%', padding: '8px', border: `1px solid ${colors.border.default}`, borderRadius: '6px', fontSize: '12px' }}
            >
  ...
  const renderBannerConfig = () => {
    ...
    const handleNestedConfigUpdate = (parent: 'animation', field: string, value: any) => {
      const parentObj = config[parent] as any;
      ...
      <div style={{ marginBottom: '20px', paddingBottom: '16px', borderBottom: `1px solid ${colors.border.default}` }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
          <button
            ...
            style={{
              padding: '12px',
              border: `2px solid ${(config.mode || 'default') === 'default' ? colors.primary[500] : colors.border.default}`,
              borderRadius: '8px',
        ...
        <div style={{ marginBottom: '20px', paddingBottom: '16px', borderBottom: `1px solid ${colors.border.default}` }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            <button
              ...
              style={{
                padding: '8px',
                border: `1px solid ${config.position === 'top' ? colors.primary[500] : colors.border.default}`,
                borderRadius: '6px',
            ...
            <button
              ...
              style={{
                padding: '8px',
                border: `1px solid ${config.position === 'bottom' ? colors.primary[500] : colors.border.default}`,
                borderRadius: '6px',
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
            <button
              ...
              style={{
                flex: 1,
                padding: '8px',
                border: `1px solid ${config.height === 'auto' ? colors.primary[500] : colors.border.default}`,
                borderRadius: '6px',
            ...
            <button
              ...
              style={{
                flex: 1,
                padding: '8px',
                border: `1px solid ${typeof config.height === 'number' ? colors.primary[500] : colors.border.default}`,
                borderRadius: '6px',
          ...
          config.mode === 'image-only' && (
            <div style={{ marginBottom: '20px', paddingBottom: '16px', borderBottom: `1px solid ${colors.border.default}` }}>
              ...
              <label style={{
                ...
                <span style={{ fontSize: '10px', color: colors.text.tertiary, marginTop: '4px' }}>
                  PNG, JPG, GIF (Max 5MB)
                </span>
                <input
                  type="file"
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            <div>
              ...
              <input
                ...
                style={{ width: '100%', padding: '6px', border: `1px solid ${colors.border.default}`, borderRadius: '4px', fontSize: '12px' }}
              />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '11px', color: colors.text.secondary, marginBottom: '4px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button
                  ...
                  style={{
                    flex: 1,
                    padding: '8px',
                    border: `1px solid ${config.shape === 'circle' ? colors.primary[500] : colors.border.default}`,
                    borderRadius: '6px',
                ...
                <button
                  ...
                  style={{
                    flex: 1,
                    padding: '8px',
                    border: `1px solid ${config.shape === 'rectangle' ? colors.primary[500] : colors.border.default}`,
                    borderRadius: '6px',
        ...
        <div style={{ marginBottom: '20px', paddingBottom: '16px', borderBottom: `1px solid ${colors.border.default}` }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            <div>
              ...
              <input type="number" value={config.offsetX || 20} onChange={(e) => handleConfigUpdate('offsetX', Number(e.target.value))} style={{ width: '100%', padding: '4px', border: `1px solid ${colors.border.default}`, borderRadius: '4px' }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '11px', color: colors.text.secondary, marginBottom: '4px' }}>Offset Y</label>
              <input type="number" value={config.offsetY || 20} onChange={(e) => handleConfigUpdate('offsetY', Number(e.target.value))} style={{ width: '100%', padding: '4px', border: `1px solid ${colors.border.default}`, borderRadius: '4px' }} />
            </div>
          </div>
        </div>
        ...
        <div style={{ marginBottom: '20px', paddingBottom: '16px', borderBottom: `1px solid ${colors.border.default}` }}>
          ...
          <div style={{ marginBottom: '12px' }}>
            ...
            {config.glassmorphism?.enabled && (
              <div style={{ paddingLeft: '8px', borderLeft: `2px solid ${colors.border.default}` }}>
                <div style={{ marginBottom: '8px' }}>
          ...
          <div>
            ...
            {config.gradient?.enabled && (
              <div style={{ paddingLeft: '8px', borderLeft: `2px solid ${colors.border.default}`, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                <div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '4px' }}>
              <select
                ...
                style={{ flex: 1, padding: '6px', border: `1px solid ${colors.border.default}`, borderRadius: '6px', fontSize: '12px', outline: 'none' }}
              >
              ...
              {selectedLayerObj?.style?.width !== 'auto' && selectedLayerObj?.style?.width !== undefined && (
                <>
                  <input
                    ...
                    style={{ width: '50px', padding: '6px', border: `1px solid ${colors.border.default}`, borderRadius: '6px', fontSize: '12px', outline: 'none' }}
                  />
                  <select
                    ...
                    style={{ width: '45px', padding: '6px', border: `1px solid ${colors.border.default}`, borderRadius: '6px', fontSize: '12px', outline: 'none' }}
                  >
          ...
          <div style={{ marginBottom: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '4px' }}>
              <input
                type="number"
                value={(() => {
                  const val = selectedLayerObj?.style?.transform?.translateX;
                ...
                onChange={(e) => {
                  ...
                  handleStyleUpdate('transform', { ...currentTransform, translateX: isPercent ? `${numVal}%` : Number(numVal) });
                }}
                style={{ flex: 1, padding: '8px', border: `1px solid ${colors.border.default}`, borderRadius: '6px', fontSize: '12px', outline: 'none' }}
              />
              <button
                onClick={() => {
                  const currentTransform = selectedLayerObj?.style?.transform || {};
                  const val = currentTransform.translateX;
                  const valStr = val !== undefined ? String(val) : '0';
                  const isPercent = valStr.includes('%');
                  const numVal = valStr.replace(/[^0-9.-]/g, '');
                  const newUnit = isPercent ? 'px' : '%';
                  handleStyleUpdate('transform', { ...currentTransform, translateX: newUnit === '%' ? `${numVal}%` : Number(numVal) });
                }}
                style={{
                  padding: '0 8px',
                  border: `1px solid ${colors.border.default}`,
              ...
              >
                {String(selectedLayerObj?.style?.transform?.translateX || '').includes('%') ? '%' : 'px'}
              </button>
            </div>
          </div>
          ...
          <div style={{ marginBottom: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '4px' }}>
              <input
                type="number"
                value={(() => {
                  const val = selectedLayerObj?.style?.transform?.translateY;
                  if (val === undefined) return 0;
                  return String(val).replace(/[^0-9.-]/g, '');
                })()}
                onChange={(e) => {
                  ...
                  const isPercent = String(val).includes('%');
                  const numVal = e.target.value;
                  handleStyleUpdate('transform', { ...currentTransform, translateY: isPercent ? `${numVal}%` : Number(numVal) });
                }}
                style={{ flex: 1, padding: '8px', border: `1px solid ${colors.border.default}`, borderRadius: '6px', fontSize: '12px', outline: 'none' }}
              />
              <button
                onClick={() => {
                  const currentTransform = selectedLayerObj?.style?.transform || {};
                  const val = currentTransform.translateY;
                  const valStr = val !== undefined ? String(val) : '0';
                  const isPercent = valStr.includes('%');
                  const numVal = valStr.replace(/[^0-9.-]/g, '');
                  const newUnit = isPercent ? 'px' : '%';
                  handleStyleUpdate('transform', { ...currentTransform, translateY: newUnit === '%' ? `${numVal}%` : Number(numVal) });
                }}
                style={{
                  padding: '0 8px',
                  border: `1px solid ${colors.border.default}`,
              ...
              >
                {String(selectedLayerObj?.style?.transform?.translateY || '').includes('%') ? '%' : 'px'}
              </button>
            </div>
          </div>
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '12px' }}>
            <div>
              ...
              <select
                ...
                style={{ width: '100%', padding: '8px', border: `1px solid ${colors.border.default}`, borderRadius: '6px', fontSize: '13px', outline: 'none' }}
              >
  ...
  // Checkbox properties
  if (selectedLayerObj.type === 'checkbox') {
    const label = selectedLayerObj.content?.checkboxLabel || 'I agree';
    const checked = selectedLayerObj.content?.checked || false;
    const checkboxColor = selectedLayerObj.content?.checkboxColor || '#6366F1';
    const textColor = selectedLayerObj.content?.textColor || '#374151';
    ...
    return (
      <>
        <div style={{ marginBottom: '20px' }}>
          <h5 style={{ margin: '0 0 12px 0', fontSize: '13px', fontWeight: 600, color: colors.text.primary }}>☑️ Checkbox Properties</h5>
          <div style={{ marginBottom: '12px' }}>
            <label style={{ display: 'block', fontSize: '12px', color: colors.text.secondary, marginBottom: '4px' }}>Label</label>
            <input
              type="text"
              value={label}
              onChange={(e) => handleContentUpdate('checkboxLabel', e.target.value)}
  ...
  if (selectedLayerObj.type === 'rating') {
    const maxStars = selectedLayerObj.content?.maxStars || 5;
    const rating = selectedLayerObj.content?.rating || 4;
    const starColor = selectedLayerObj.style?.starColor || '#FFB800';
    const emptyStarColor = selectedLayerObj.style?.emptyStarColor || '#D1D5DB';
      ...
      <>
        <div style={{ marginBottom: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '12px' }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '12px', color: colors.text.secondary, marginBottom: '4px' }}>Default Rating</label>
              <input
                type="number"
                step="0.5"
                value={rating}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '12px' }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '12px', color: colors.text.secondary, marginBottom: '4px' }}>Empty Color</label>
              <input
                type="color"
                value={emptyStarColor}
                onChange={(e) => handleStyleUpdate('emptyStarColor', e.target.value)}
                style={{ width: '100%', height: '40px', border: `1px solid ${colors.border.default}`, borderRadius: '6px', cursor: 'pointer' }}
              />
            </div>
          </div>
...
const handleNudgeTypeSelect = (id: string) => {
  ...
  createCampaign(
    selectedExperienceType as any || 'nudges',
    id as any
  );
  ...
  if (currentCampaign?.id) {
    navigate(`/campaign-builder?id=${currentCampaign.id}&experience=${selectedExperienceType || 'nudges'}`, { replace: true });
  }
